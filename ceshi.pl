...
my $fileRegex = "Club";           # 目标文件
my $methodName = "updateReleaseState2";    # 方法名
my $beanName = "Release";            # bean
my $methodNote = "更新线路状态";  # 方法注释
my $methodNote2 = "";                  # 分页注释(获取...数目)
my $argNote = "线路信息";
#方法类型 1:get rvo,2:insert,3:update,4:get list,5:get int, 6:get pager
my $method_type = "3";

my $insert_id = "messageID";           # 插入,count时的主键
my $tab_name = "T_F1_RELEASE";         # 表名
...
sub get_sql{
    ...
    #dto塞值
    my $param_mould5 = "        dto.setsuper_column_name(${lowerBeanName}VO.getcolumn_name());";
...
sub get_mould{
    ...
    my $param_type = "${beanName}VO";       #方法中参数类型
    my $param_alias = "${lowerBeanName}VO"; #方法中参数别名
...
#分页用
        ...
        $bizImpl_code = "// 获取sum
        int sum = ${lowerFileRegex}Dao.${methodName}Count(${param_alias});

        ${param_alias}.getBeginEnd();
        // 获取List
        List<${beanName}RVO> list = ${lowerFileRegex}Dao.${methodName}List(${param_alias});
     ...
     */
    \@Override
    public $out_param2 get${beanName}Count($in_param) throws SQLException {

        log.info(GlobalContract.LOG_BEGIN);

        ${dao_sql_return2}this.getSqlMapClientTemplate().$dao_query_type2(\"${lowerFileRegex}.get${beanName}Count\",$note_param);
    ...
    public $out_param2 get${beanName}Count($in_param) throws SQLException;\n";
        ...
        my $xmlMould2 = "\n    <!-- $methodNote2 -->
    <$xml_ibatis id=\"get${beanName}Count\" parameterClass=\"$note_param\" $xml_result_class2>
        $xml_sql2
    </$xml_ibatis>\n";
